...
const tooltipContent = [
  { name: "Braclets", image: '/img/preview/bracelet-gold1.png', salePrice: 500, originialPrice: 800 },
  { name: "Rings", image: '/img/preview/ring1.png', salePrice: 500, originialPrice: 800 },
  { name: "Necklace", image: '/img/preview/necklace-gold1.png', salePrice: 500, originialPrice: 800 },
  { name: "Earrings", image: '/img/preview/earring-gold1.png', salePrice: 500, originialPrice: 800 },
]

export default function TrendingCollections() {
  const [selectedCategory, setSelectedCategory] = useState('rings');
  const { data, isLoading } = useFetch(`${baseUrl}/api/v1/product/get-trending?categoryName=${selectedCategory}&limit=4`, {}, [])
  ...
  return (
    <div className='xs:gap-3 3xl:max-h-[840px] flex flex-col gap-2 sm:gap-6 lg:max-h-[700px] lg:flex-row lg:items-stretch lg:justify-center lg:gap-4 xl:max-h-[760px] 2xl:gap-6'>
      ...
      {isLoading ? <></> : (<div className='xl:grid-row-2 grid grid-cols-2 gap-4 sm:gap-4 lg:w-[22%] lg:grid-cols-1 2xl:gap-6'>
        {data.products[0] && (
          <PreviewCardMd key={data.products[0]._id} product={data.products[0]} />
        )}
        {data.products[1] && (
          <PreviewCardMd key={data.products[1]._id} product={data.products[1]} />
        )}
      </div>)}
      ...
      {isLoading ? <></> : (<div className='xl:grid-row-2 grid grid-cols-2 gap-4 sm:gap-4 lg:w-[22%] lg:grid-cols-1 2xl:gap-6'>
        {data.products[2] && (
          <PreviewCardMd key={data.products[2]._id} product={data.products[2]} />
        )}
        {data.products[3] && (
          <PreviewCardMd key={data.products[3]._id} product={data.products[3]} />
        )}
      </div>)}
    </div>
  );
